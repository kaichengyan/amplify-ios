...
extension AmplifyInProcessReportingOperation {
    /// A Publisher that emits in-process values for an operation, or the associated failure. Cancelled operations will
    /// emit a completion without a value as long as the cancellation was received before the operation was resolved.
    ///
    /// Note that the `inProcessPublisher`'s `Failure` type is `Never`. An AmplifyOperation reports its overall success
    ...
    /// Publish an in-proceess value for the operation
    ///
